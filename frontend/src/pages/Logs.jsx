...
import {
  Radio, ArrowLeft, Search, Download, Trash2, ShieldAlert, Activity, RefreshCw,
} from "lucide-react";
...
export function CategoryBadge({ category }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 font-mono-data text-[10px] tracking-[0.1em] px-2 py-1 border ${
      security ? "border-[var(--ossm-cyan)]/40 text-[var(--ossm-cyan)]" : "border-amber-400/40 text-amber-300"
    ...
    <div className="relative z-10 min-h-screen max-w-5xl mx-auto px-5 sm:px-8 py-6" data-testid="logs-page">
      <header className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-mono-data text-xs text-[var(--ossm-muted)] ml-2 hidden sm:inline">ACTIVITY LOG</span>
        </div>
        <button onClick={() => navigate("/admin")} data-testid="logs-back-button"
          className="flex items-center gap-1.5 font-mono-data text-xs text-[var(--ossm-text-2)] hover:text-[var(--ossm-cyan)] transition-colors">
      ...
      <div className="hud-panel p-5 sm:p-6 mb-6">
        <div className="flex flex-col lg:flex-row lg:items-end gap-4">
          <div className="flex-1">
            <label className="font-display text-xs tracking-[0.15em] text-[var(--ossm-text-2)]">SEARCH</label>
            <div className="relative mt-2">
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--ossm-muted)]" />
              <input value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && applyFilters()}
                ...
                className="w-full bg-[var(--ossm-base)] border border-[var(--ossm-overlay)] pl-9 pr-3 py-2.5 font-mono-data text-sm outline-none focus:border-[var(--ossm-cyan)] transition-colors" />
            </div>
          </div>
          <div>
            <label className="font-display text-xs tracking-[0.15em] text-[var(--ossm-text-2)]">CATEGORY</label>
          ...
          <div>
            <label className="font-display text-xs tracking-[0.15em] text-[var(--ossm-text-2)]">FROM</label>
            <input type="date" value={start} onChange={(e) => setStart(e.target.value)} data-testid="log-filter-start"
              className="w-full mt-2 bg-[var(--ossm-base)] border border-[var(--ossm-overlay)] px-3 py-2.5 font-mono-data text-sm outline-none focus:border-[var(--ossm-cyan)] transition-colors" />
          </div>
          <div>
            <label className="font-display text-xs tracking-[0.15em] text-[var(--ossm-text-2)]">TO</label>
          ...
          <button onClick={applyFilters} data-testid="log-filter-apply"
            className="flex items-center justify-center gap-2 bg-[var(--ossm-cyan)] text-[var(--ossm-base)] font-display font-bold text-xs tracking-[0.1em] px-5 py-3 active:scale-95 transition-transform">
            <RefreshCw size={14} /> APPLY
          </button>
        </div>

        <div className="flex flex-wrap items-center justify-between gap-3 mt-5 pt-5 border-t border-[var(--ossm-overlay)]">
          <span className="font-mono-data text-xs text-[var(--ossm-muted)]" data-testid="log-total">{total} event{total !== 1 ? "s" : ""}</span>
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={clearAll} data-testid="log-clear-button"
              className="flex items-center gap-1.5 border border-[var(--ossm-overlay)] px-3 py-2 font-mono-data text-xs text-[var(--ossm-text-2)] hover:border-[var(--ossm-danger)] hover:text-[var(--ossm-danger)] transition-colors">
      ...
      <div className="hud-panel overflow-hidden" data-testid="logs-table">
        {items.length === 0 && !loading && (
          <p className="font-mono-data text-sm text-[var(--ossm-muted)] py-16 text-center">No activity yet.</p>
        )}
        <div className="divide-y divide-[var(--ossm-overlay)]">
          {items.map((it) => (
            <div key={it.id} data-testid={`log-row-${it.action}`} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 px-4 sm:px-6 py-3.5 hover:bg-[var(--ossm-base)]/40 transition-colors">
              <span className="font-mono-data text-xs text-[var(--ossm-muted)] w-40 shrink-0">{fmtTs(it.ts)}</span>
              <CategoryBadge category={it.category} />
              <span className="font-display text-sm text-white flex-1 min-w-0 truncate">
                {labelFor(it.action)}
                {it.target && <span className="text-[var(--ossm-cyan)] font-mono-data ml-2">{it.target}</span>}
              </span>
              <span className="font-mono-data text-xs text-[var(--ossm-text-2)] truncate max-w-[220px]">{it.actor}</span>
              {detailText(it.detail) && (
                <span className="font-mono-data text-[11px] text-[var(--ossm-muted)] truncate max-w-[220px]">{detailText(it.detail)}</span>
              )}
            </div>
          ))}
        </div>
      ...
      {items.length < total && (
        <div className="flex justify-center mt-6">
          <button onClick={() => load(false)} disabled={loading} data-testid="log-load-more"
            className="border border-[var(--ossm-overlay)] px-6 py-3 font-mono-data text-xs hover:border-[var(--ossm-cyan)]/50 hover:text-[var(--ossm-cyan)] transition-colors disabled:opacity-50">
