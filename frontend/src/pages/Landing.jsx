...
import { Radio, ArrowRight, ShieldCheck, Timer, Bluetooth } from "lucide-react";
    ...
    <div className="relative min-h-screen z-10 flex flex-col">
      <header className="flex items-center justify-between px-6 sm:px-10 py-6">
        ...
        <a
          ...
          className="font-mono-data text-xs text-[var(--ossm-muted)] hover:text-[var(--ossm-cyan)] transition-colors"
        >
          OWNER ACCESS
        </a>
      </header>
      ...
      <main className="flex-1 flex flex-col justify-center max-w-5xl w-full mx-auto px-6 sm:px-10 py-12">
        <div className="fade-up max-w-3xl">
          <span className="font-mono-data text-xs tracking-[0.2em] text-[var(--ossm-cyan)] border border-[var(--ossm-cyan)]/30 px-3 py-1.5">
          ...
          <h1 className="font-display font-black uppercase tracking-[0.03em] text-4xl sm:text-5xl lg:text-6xl mt-8 leading-[1.05]">
            Hand over the<br />
            <span className="text-[var(--ossm-cyan)] text-glow-cyan">controls</span>, on your terms.
          </h1>
          <p className="text-[var(--ossm-text-2)] text-base sm:text-lg mt-6 max-w-xl leading-relaxed">
        ...
        <form onSubmit={enter} className="fade-up hud-panel mt-12 p-6 sm:p-8 max-w-xl" style={{ animationDelay: "0.1s" }}>
          <label className="font-display text-xs tracking-[0.2em] text-[var(--ossm-text-2)]">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-4">
            <input
              ...
              className="flex-1 bg-[var(--ossm-base)] border border-[var(--ossm-overlay)] px-4 py-3.5 font-mono-data text-xl tracking-[0.3em] uppercase outline-none focus:border-[var(--ossm-cyan)] transition-colors placeholder:text-[var(--ossm-muted)] placeholder:tracking-normal placeholder:text-base"
            />
            <button
              type="submit"
              data-testid="enter-code-button"
              className="bg-[var(--ossm-cyan)] text-[var(--ossm-base)] font-display font-bold tracking-[0.15em] px-6 py-3.5 flex items-center justify-center gap-2 active:scale-95 transition-transform"
            >
        ...
        <div className="grid sm:grid-cols-3 gap-4 mt-12 max-w-3xl">
          {[
            ...
            <div key={f.t} className="hud-panel p-5">
              <f.icon className="text-[var(--ossm-cyan)]" size={20} />
              <h3 className="font-display tracking-[0.1em] text-sm mt-3">{f.t}</h3>
              <p className="text-[var(--ossm-text-2)] text-sm mt-1.5 leading-relaxed">{f.d}</p>
            </div>
          ))}
        </div>
      </main>
