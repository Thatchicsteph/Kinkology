...
import { Radio, Lock, ShieldCheck, KeyRound, UserPlus, Loader2 } from "lucide-react";
...
export default function AdminLogin() {
  ...
  return (
    <div className="relative z-10 min-h-screen flex items-center justify-center px-6">
      {needsSetup === null ? (
        <div className="flex items-center gap-3 text-[var(--ossm-text-2)]" data-testid="setup-checking">
          <Loader2 className="animate-spin text-[var(--ossm-cyan)]" size={22} />
        ...
        <form onSubmit={submitSetup} className="hud-panel w-full max-w-md p-8 fade-up" data-testid="setup-form">
          ...
          <h1 className="font-display font-black uppercase tracking-[0.05em] text-2xl mt-6 flex items-center gap-2">
            <UserPlus size={20} className="text-[var(--ossm-cyan)]" /> Create Owner Account
          </h1>
          <p className="text-[var(--ossm-text-2)] text-sm mt-2">First launch — set up the owner account for this device.</p>

          {error && (
            <div className="mt-5 border border-[var(--ossm-danger)]/40 bg-[var(--ossm-danger)]/10 text-[var(--ossm-danger)] text-sm px-4 py-3" data-testid="setup-error">
              {error}
            </div>
          )}
          ...
          <div className="mt-6 space-y-4">
            ...
            <div>
              ...
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} data-testid="setup-password" required minLength={8}
                className="w-full mt-2 bg-[var(--ossm-base)] border border-[var(--ossm-overlay)] px-4 py-3 outline-none focus:border-[var(--ossm-cyan)] transition-colors" />
              <p className="text-[var(--ossm-muted)] text-xs mt-1.5">At least 8 characters.</p>
            </div>
            <div>
