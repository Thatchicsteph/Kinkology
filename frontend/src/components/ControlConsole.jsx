    ...
    <div className={`px-1 ${disabled ? "opacity-40 pointer-events-none" : ""}`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-display text-xs tracking-[0.15em] text-[var(--ossm-text-2)]">{label}</span>
          {limitNote && (
            <span className="flex items-center gap-1 font-mono-data text-[10px] text-[var(--ossm-muted)]">
...
export function ControlConsole({ onCommand, disabled = false, limits = { min_depth: 0, max_speed: 100 } }) {
  ...
  return (
    <div className="space-y-8" data-testid="control-console">
      ...
      <div className={disabled ? "opacity-40 pointer-events-none" : ""}>
        <span className="font-display text-xs tracking-[0.15em] text-[var(--ossm-text-2)] flex items-center gap-2 mb-3">
          <Zap size={14} className="text-[var(--ossm-cyan)]" /> AUTO PROGRAMS
        </span>
        <div className="grid grid-cols-2 gap-2">
          {PROGRAMS.map((p) => (
            <button
              ...
              className={`text-left px-3 py-2.5 border transition-colors duration-200 ${
                activeProgram === p.id
                  ? "border-[var(--ossm-cyan)] bg-[var(--ossm-cyan)]/[0.12] text-white glow-cyan"
                  : "border-[var(--ossm-overlay)] text-[var(--ossm-text-2)] hover:border-[var(--ossm-cyan)]/40"
              }`}
            >
              <span className="block text-sm font-medium">{p.name}</span>
              <span className="block font-mono-data text-[10px] text-[var(--ossm-muted)] mt-0.5">{p.desc}</span>
            </button>
          ))}
        </div>
        {activeProgram && (
          <p className="font-mono-data text-xs text-[var(--ossm-cyan)] mt-3" data-testid="program-active-note">
